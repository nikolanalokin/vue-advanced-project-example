...
export default {
  ...
  computed: {
    ...
    fieldClass () {
      return `n-${this.isTextarea === true ? 'textarea' : 'input'}` +
        (this.autogrow === true ? ' n-textarea_autogrow' : '') + (!this.value ? ' is-empty' : '')
    }
  },
  ...
  methods: {
    ...
    __getControl (h) {
      ...
      return h(this.isTextarea ? 'textarea' : 'input', {
        ref: 'input',
        staticClass: 'n-field__native n-placeholder',
