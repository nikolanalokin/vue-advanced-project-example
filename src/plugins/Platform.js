...
export default {
  ...
  install ({ Vue, queues }) {
    if (isSSR) {
      queues.server.push((n, ctx) => {
        n.platform = this.parseSSR(ctx.ssr)
      })
      return
    }
    ...
    if (fromSSR) {
      queues.takeover.push(n => {
        onSSR = fromSSR = false
        Object.assign(n.platform, getClientProperties())
      })
      Vue.util.defineReactive(Vue.prototype, '$platform', this)
    }
    else {
